...
class _DateRangePickerState extends State<DateRangePicker> {
  ...
  Widget _buildSelectionHint() {
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          Icon(_isSelectingEndDate ? Icons.event : Icons.touch_app_rounded, size: 16, color: color),
        ...
        children: CalendarDateUtils.weekdayLabels.map((label) {
          ...
          return Expanded(
            child: Center(
              child: Text(
                label,
                style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: isWeekend ? AppColors.textTertiary : AppColors.textSecondary),
              ),
            ),
          );
        }).toList(),
  ...
  Widget _buildDayCell(DateTime date) {
    ...
    final isStart = _startDate != null && CalendarDateUtils.isSameDay(date, _startDate!);
    final isEnd = _endDate != null && CalendarDateUtils.isSameDay(date, _endDate!);
    final isInRange = _isInSelectedRange(date);
    final isSingleDay = _startDate != null && _endDate != null && CalendarDateUtils.isSameDay(_startDate!, _endDate!);

    return GestureDetector(
      onTap: isInMonth ? () => _onDateTap(date) : null,
      child: Container(
        margin: const EdgeInsets.symmetric(vertical: 2),
        decoration: _getDayDecoration(isStart: isStart, isEnd: isEnd, isInRange: isInRange, isSingleDay: isSingleDay),
        child: Center(
          child: Container(
            width: 36,
            height: 36,
            decoration: _getDayCircleDecoration(isStart: isStart, isEnd: isEnd, isToday: isToday, isInMonth: isInMonth),
            child: Center(
              child: Text(
                '${date.day}',
                style: TextStyle(
                  ...
                  color: _getDayTextColor(isStart: isStart, isEnd: isEnd, isToday: isToday, isInMonth: isInMonth, isInRange: isInRange),
                ),
              ),
            ),
          ),
  ...
  BoxDecoration? _getDayDecoration({required bool isStart, required bool isEnd, required bool isInRange, required bool isSingleDay}) {
    ...
    if (isInRange) {
      return BoxDecoration(color: AppColors.gradientStart.withValues(alpha: 0.15));
    }

    return null;
  }

  BoxDecoration? _getDayCircleDecoration({required bool isStart, required bool isEnd, required bool isToday, required bool isInMonth}) {
    if (isStart || isEnd) {
      return BoxDecoration(
        gradient: AppColors.primaryGradient,
        shape: BoxShape.circle,
        boxShadow: [BoxShadow(color: AppColors.gradientStart.withValues(alpha: 0.4), blurRadius: 6, offset: const Offset(0, 2))],
      );
    }
  ...
  Color _getDayTextColor({required bool isStart, required bool isEnd, required bool isToday, required bool isInMonth, required bool isInRange}) {
    if (isStart || isEnd) {
      return Colors.white;
    }
  ...
  Widget _buildSelectedRangeDisplay() {
    return Padding(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          gradient: LinearGradient(colors: [AppColors.gradientStart.withValues(alpha: 0.1), AppColors.gradientEnd.withValues(alpha: 0.1)]),
  ...
  Widget _buildDateDisplay({required String label, required DateTime? date, required bool isStart}) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(8),
        boxShadow: isActive ? [BoxShadow(color: AppColors.shadow.withValues(alpha: 0.1), blurRadius: 4, offset: const Offset(0, 2))] : null,
      ),
      child: Column(
        children: [
          Text(
...
/// 顯示日期範圍選擇器對話框
Future<DateTimeRange?> showDateRangePickerDialog(BuildContext context, {required DateTime initialStartDate, required DateTime initialEndDate}) async {
